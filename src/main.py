...
def send_approval_digests(dry_run: bool) -> None:
    ...
    async def _run() -> None:
        ...
        # off the count so we don't ship "1 variants".
        label_specs: dict[str, tuple[str, str, str]] = {
            "new_variant": ("NEW VARIANT", "NEW VARIANTS", "Generated by the agent — launch or decline."),
            "pause_variant": ("AD TO PAUSE", "ADS TO PAUSE", "Statistically losing or fatigued."),
            "scale_budget": ("BUDGET CHANGE", "BUDGET CHANGES", "Thompson sampling reallocation."),
        ...
        for _, entry in per_user.items():
            ...
            if ok:
                ...
                failed.append((owner_email, "sendgrid non-2xx"))
                click.echo(f"  ✗ {owner_email}: sendgrid failed")
